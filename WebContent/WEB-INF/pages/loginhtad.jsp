...
<style>
	*{font-size: 9pt;font-family: "Microsoft YaHei","sans-serif";}
	img{width:112px; height:44px; line-height:46px;cursor: pointer;border:1px solid #ccc; }
...
<!-- 	    <li class="yzm">
	    <span><input name="yzm" maxlength="4"  type="text" value="验证码" onclick="JavaScript:this.value=''"/></span><img id="code" alt="验证码" src="../idCode" width="100" height="32" class="passcode" onclick="this.src+='?'" title="看不清？点击换一个" /> 
	    </li> -->
	    <li style=''><input name="" type="button" class="loginbtn" value="登录"  onclick="fn_dl(this)"  /></li>
	    </ul>
    </form>
	...
	<script>
		...
		$(function(){
			...
			if (document.referrer.indexOf("yanshi")!=-1){
				layidx = layer.msg(last+'秒后将为您自动登录',{offset:'t', time:6000, closeBtn:1})
		...
		function fn_dl(tag){
			...
			$.ajax({
				...
				success:function(data){
					layer.closeAll();
					if (data == "0"){
						location.href = '${path}/nedadm/index.dhtml';
					}else{
