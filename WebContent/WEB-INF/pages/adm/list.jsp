...
<head>
	...
	<style>
		...
		.cxqu{background: #f5f5f5; float:left;border-radius: 4px;padding: 5px 0;}
		.cxqu li{float: left;margin-right: 0px;line-height: 40px;vertical-align: middle;font-weight: bold; padding:0 9px 4px; }
...
<body>
	<input type="hidden" id="id_listcurpg" value="${o.r.pg}"/>
										...
										<td>
											<c:if test="${!empty(extwz)}">
												<c:if test="${!zd.exttail}">
													...
													<div style="margin:0px;overflow: hidden"><img style="max-height:80px;" src="${path}/upload/pic/${itm[zd.zdm]}" /></div>
												</c:if>
												<c:if test="${zd.exttail}">
													<div style="margin:0px;overflow: hidden"><img style="max-height:80px;" src="${path}/upload/pic/${itm[zd.zdm]}" /></div>
													${zd.exts}${extwz}${zd.exte}
											...
											<c:if test="${empty(extwz)}"><div style="margin:0px;overflow: hidden"><img style="max-height:80px;" src="${path}/upload/pic/${itm[zd.zdm]}" /></div></c:if>
										</td>
									</c:if>
									<c:if test="${empty(itm[zd.zdm])}">
								...
								<c:when test="${zd.zdType=='text'}"><!-- text字段 -->
									<c:choose>
										...
										<c:when test="${!empty(zd.textmap) && !empty(zd.textmpzd)}">
											<c:if test="${!empty(zd.textmap[fn:trim(itm[zd.textmpzd])])}"><c:set var="celltext" value="${zd.textmap[fn:trim(itm[zd.textmpzd])]}"/></c:if>
											<c:if test="${empty(zd.textmap[fn:trim(itm[zd.textmpzd])])}"><c:set var="celltext" value="${zd.empcoltip}"/></c:if>
											<c:if test="${!empty(extwz)}">
												<c:if test="${zd.exttail}"><c:set var="celltext" value="${celltext}${extwz}"/></c:if>
												<c:if test="${!zd.exttail}"><c:set var="celltext" value="${extwz}${celltext}"/></c:if>
											</c:if>
					...
					function() {
						$(".clsselinpt").each(function(){
							if ($(this).children("div").length>0){
								$(this).children("input").animate({width:182-$(this).children("div").width()},200);//查询区对齐的
							}
						})
					}, 100
				) 
