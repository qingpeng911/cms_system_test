		...
		<tbody>
			{{each dyc as v idx}}
				<tr {{if idx%2!=0}}style='background:rgb(255, 252, 234)'{{/if}}>
					...
					<td>
						<a href="javascript:" onclick="fn_edit(this,'{{v.zdxmc}}')">编辑</a>
						 {{if v.cnt<=0}}| <a href="javascript:" onclick="fn_del(this)">删除</a>{{/if}}
					</td>
				</tr>
			{{/each}}
		</tbody>
		...
		function fn_add(){
			...
			if (treeObj !=null){
				...
				if (nodes.length <=0){
					...
					var data={root:nodes[0].name,pId:nodes[0].id, px:px,title:'新增栏目',sel:g_sel,exobj:{}}
		...
		function fn_edit(tag,pzdxmc){
			...
			if (selnode != null) {
				...
				var data={root:selnode.name,pId:selnode.id, lmmc:$(tag).parent().parent().children("td:eq(3)").text(), innav:innav,
						  px:$(tag).parent().parent().children("td:eq(6)").text(),lx:pzdxmc, id:id,title:'编辑栏目', sel:g_sel,exobj:exobj}
				var html = template('sid_addedit', data);
				document.getElementById('rgcon').innerHTML =html;
			}
		}
