								...
								<c:when test="${t.type=='PIC'}">
									<c:if test="${lx!=2 &&  o.r.need}">
									 	<td class="upload ${t.validate}" zdm="${t.zdm}">
									 		...
									 		<input type="file" <c:if test="${lx==2 || !o.r.need}">disabled readonly</c:if> class="input-text lh25" name="file"/>
									 		<input type="button" value="上传" onclick="fn_sc(this)" class="ext_btn ext_btn_submit"/>
									 		<input type="button" value="清空" onclick="fn_qk(this)" class="ext_btn"/>
									 		<div class="tishi">请选择jpg、png或gif格式图片，大小不超过3M</div>
										</td>
									</c:if>
								 	<td colspan="${(lx==2 || !o.r.need)?2:1}" style="overflow:auto;">
								 		<c:if test="${empty(obj[t.zdm])}">未上传图片</c:if>
								 		<c:if test="${!empty(obj[t.zdm])}"><img src='${path}/upload/pic/${obj[t.zdm]}'></img></c:if>
								 	</td>
								</c:when>
									...
									<td colspan="2" onselectstart="return false">
										<select name="zdb" class="select" <c:if test="${lx==2 || !o.r.need}">disabled readonly</c:if> 
											...
											<c:if test="${t.zdbHaveZdy}">
												<option value='' <c:if test="${lx!=0 && empty(obj['zdb']) && !empty(obj[t.zdm])}">selected</c:if>>自定义</option>
											</c:if>
										</select>
										<select name="${t.zdm}" class="select" <c:if test="${lx==2 || !o.r.need}">disabled readonly</c:if>
								...
								<c:when test="${t.type=='DATE'}">
									<td colspan="2">
										...
										<c:if test="${t.title=='更新日期' && t.zdm=='gx' && !(lx==2 || !o.r.need)}">
											...
											<input type="checkbox" id="chkctgx"/>如需编辑更新日期请先选中
										</c:if>
									</td>
								</c:when>
							</c:choose>
    ...
    <script type="text/javascript">
		$(function(){
			...
			<c:if test="${!empty(havetextarea)}">
				$("textarea").keyup(function(){
					...
					if (len>maxlen){
						layer.msg('【'+mc+'】字段超出最大保存长度('+maxlen+')，请检查输入内容！')
					}else if (len==maxlen){
					...
					if (ipt.length>0){//是input
						...
						if (clen> maxlen){//如果填了就要检查长度
							var mc = rich.parent().prev().text()
		                   	invalid = '【'+mc+'】字段超出最大保存长度('+maxlen+')，请检查输入内容！'
		                   	kj = ueobj
		                    return false;
						}
					}else if (td.attr("class") && td.attr("class").indexOf("upload")!=-1){//是图片
						var cls = td.attr('class')
						if (cls.indexOf('required') != -1 && $.trim(td.next().text())=='未上传图片'){
							var mc = td.prev().text()
		                   	invalid = '【'+mc+'】必须上传'
			...
			function fn_sc(tag){
				...
				var options = {
			            ...
			            success : function(json) {
			            	if(json.ret=="0"){
			            		layer.closeAll();
				            	$(tag).parent().parent().next().empty().append("<img src='${path}/upload/pic/"+json.src+"'></img>")
			            	}else{
			            		fn_qk($(tag).next());
			            		layer.msg(json.ret);
			            	}
		...
		<c:if test="${!o.r.need}">
			$(function(){
				...
				$("#id_ae_form .clsinput,#id_ae_form .clstextarea").each(function(idx){
		...
		<c:if test="${!empty(havezdb)}">
			function fn_zdbchange(tag,maxlen, zdm, subv){
				...
				if (isEmptyStr(val)){
					...
					if (cdsize==2) $(tag).parent().append("<input value='"+subv+"' type='text' style='width:20% !important' class='input-text lh25' maxlength='"+maxlen+"' name='"+zdm+"' />");
					else {
