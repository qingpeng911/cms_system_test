...
<body onselectstart="return false;" >
    <div class="top">
    	...
    	<div class='tright'>
    		...
    		<span class="icon-cguanliyuan" <c:if test="${wdxxs>0}">style="margin-right:0"</c:if>>${ses_admin}</span>
   			<c:if test="${wdxxs>0 }"><span class="nav-counter" title="${wdxxs}条未读消息">${wdxxs}</span></c:if>
    		<a href="javascript:" onclick="fn_adtc()" class="icon-tuichu" target="_blank">退出</a> 
    	</div>
    </div>
	...
	<div class="center">
		<div class="left" onselectstart="return false" >
			...
			<div class="lmnu">
				<ul>
					<li>
						<div class="biaoti">配置网站<span></span></div>
    ...
    <script type="text/javascript">
    	...
    	//点击树的节点
    		...
    		$(".tab>ul>li").each(function(d){
    			...
    			if (!found && $(this).attr("id")=="tab_li_"+treeNode.id){//如果已经有了，要重新刷新一下，因为这边点的是菜单
    				...
    				$("."+$(this).attr("id")).remove();
    				loadiFrame(treeNode.lxpy, treeNode.id,treeNode.para);
    			}
    		})
    		if (!found){//还没打开过
		...
		$(function (){
			...
			$(".nav-counter").click(function(){
				zTreeOnClick(null,null,{id:10000, name:"未读消息", lxpy:"wdxx"})
			})
			...
			var setting = {
				callback: {
					onClick: zTreeOnClick,beforeClick:changeLeftSel,onExpand: leftTreeExColl, onCollapse: leftTreeExColl
				}
			};
			var zNodes =[
				{lxpy:'lanmu', id:-1,name:"栏目列表"},
				{lxpy:'yqlj',id:-2, name:"友情链接"},
				{lxpy:'zdb',id:-3, name:"字典表"},
				{name:"配置项",children:[{name:'可编辑',lxpy:'cfg_kbj',id:-41},{name:'不可编辑',lxpy:'cfg',id:-42}]},
				{lxpy:'wzjth',id:-5, name:"网站静态化"},
			];
			$.fn.zTree.init($("#treeDemo"), setting, zNodes);

      		//window.setTimeout(function() {$("#lanmutree_5_a").trigger('click')}, 300) 
		...
		function zTreeOnClklm(event, treeId, treeNode) {
			...
			if ("fulanmu_zi" == treeNode.lxpy){
				...
				if (yeziarr.length>0) treeNode.lxpy = yeziarr[0].lxpy
			}else{
				treeNode.para += "&___cid=" + treeNode.id
			}
		...
		function fn_adtc(tag){
			$.ajax({
				...
				success:function(data){
					layer.closeAll();
					location.href = "${path}/htlgn/login.dhtml";
				},
				error: function(){
